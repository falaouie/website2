...
class User {
    ...
    // Add this function inside your User class (User.php)

    public function recordWorkIn($staff_id) {
        // Get current date and time
        $work_date = date('Y-m-d');
        $work_in = getDateTime();

        // Insert into attendance table (adjust the table name as necessary)
        $query = "INSERT INTO staff_attendance (staff_id, work_date, work_in) 
                VALUES (:staff_id, :work_date, :work_in)";

        $stmt = $this->conn->prepare($query);
        $stmt->bindParam(':staff_id', $staff_id);
        $stmt->bindParam(':work_date', $work_date);
        $stmt->bindParam(':work_in', $work_in);

        return $stmt->execute(); // Return true if successful
    }

    public function recordWorkOff($staff_id) {
        // Get current date and time
        $work_off = getDateTime();
        $work_date = date('Y-m-d');

        // Fetch the work_in time for this staff member
        $query = "SELECT work_in FROM staff_attendance WHERE staff_id = :staff_id AND work_date = :work_date";
        $stmt = $this->conn->prepare($query);
        $stmt->bindParam(':staff_id', $staff_id);
        $stmt->bindParam(':work_date', $work_date);
        $stmt->execute();
        ...
        $result = $stmt->fetch(PDO::FETCH_ASSOC);
        $work_in = $result['work_in'];

        // Calculate hours worked
        $hours_worked = (strtotime($work_off) - strtotime($work_in)) / 3600; // Convert seconds to hours

        // Update work_off and hours_worked
        $update_query = "UPDATE staff_attendance SET work_off = :work_off, hours_worked = :hours_worked 
                        WHERE staff_id = :staff_id AND work_date = :work_date";

        $update_stmt = $this->conn->prepare($update_query);
        $update_stmt->bindParam(':work_off', $work_off);
        $update_stmt->bindParam(':hours_worked', $hours_worked);
        $update_stmt->bindParam(':staff_id', $staff_id);
        $update_stmt->bindParam(':work_date', $work_date);
        
        return $update_stmt->execute();
    }

    public function getAttendanceForDay($staffId, $date) {
        $query = "SELECT * FROM staff_attendance WHERE staff_id = :staff_id AND work_date = :work_date";
        $stmt = $this->conn->prepare($query);
        $stmt->bindParam(':staff_id', $staffId);
