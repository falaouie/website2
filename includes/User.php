...
class User {
    ...
    public function getCurrentFixedSchedules() {
        $query = "SELECT s.staff_id, s.first_name, s.last_name, 
                         sch.work_day, sch.start_time, sch.end_time, sch.day_off
                  FROM staff_tbl s
        ...
        // Check for open schedules (staff with no schedule entries)
        foreach ($schedules as $staffId => $staffSchedule) {
            if (empty($staffSchedule['schedule'])) {
                for ($i = 0; $i < 7; $i++) {
    ...
    public function updateFixedSchedule($data) {
        ...
        try {
            ...
            foreach ($staffIds as $staffId) {
                foreach ($days as $day) {
                    if ($openSchedule == 1) {
                        // Delete any existing schedule for open schedule
                        $deleteStmt = $this->conn->prepare("DELETE FROM schedules WHERE staff_id = :staff_id AND work_day = :work_day");
                        $deleteStmt->bindParam(':staff_id', $staffId);
                        $deleteStmt->bindParam(':work_day', $day);
                        $deleteStmt->execute();
                    } else {
                        $stmt = $this->conn->prepare("INSERT INTO schedules (staff_id, work_day, start_time, end_time, day_off) 
                                                      VALUES (:staff_id, :work_day, :start_time, :end_time, :day_off)
